...
const SUBJECTS = [
  {
    ...
    lectures: [
      { id: 1, title: "الشحنة الكهربية وقانون كولوم", code: "EM-L1", videoUrl: "https://interactive-examples.mdn.style/media/cc0-videos/flower.mp4", durationMinutes: 20 },
...
// كل سؤال ممكن يتحط له: q (نص) بس، أو image بس (سيب q فاضي أو وصف مختصر)، أو الاتنين مع بعض.
...
function LecturePlayer({ url, durationMinutes, initialWatched = 0, onProgress }) {
  if (!url) return (<div className="lu-placeholder"><Icon name="play" className="lu-mini" /> رابط الفيديو لسه هيتحط هنا</div>);
  return isDirectFile(url)
    ? <DirectVideoPlayer src={url} initialWatched={initialWatched} onProgress={onProgress} />
    : <EmbeddedVideoPlayer url={url} durationMinutes={durationMinutes} initialWatched={initialWatched} onProgress={onProgress} />;
}

function DirectVideoPlayer({ src, initialWatched, onProgress }) {
...
function EmbeddedVideoPlayer({ url, durationMinutes, initialWatched, onProgress }) {
  const furthestRef = useRef(initialWatched);
  const [displayPercent, setDisplayPercent] = useState(initialWatched);
...
/* ------------------------------------------------------------------ */
/*  فورم التسجيل — معاينة تصميم فقط (مش متصلة بقاعدة بيانات حقيقية)       */
/* ------------------------------------------------------------------ */
  ...
  return (
    <form className="reg-card" onSubmit={submit}>
      <div className="demo-banner">
        ...
        هنا معاينة لشكل فورم التسجيل بس — البيانات دي مش بتتخزن ولا بتتبعت لأي حد فعليًا في النسخة دي.
      </div>
      ...
      <button className="btn btn-amber" type="submit">إرسال الطلب للمراجعة</button>
    </form>
  );
}
...
function AdminPanel({ students, onDecide }) {
  return (
    <div className="admin-panel">
      <div className="demo-banner">
        <Icon name="shield" className="lu-mini" />
        دي معاينة لشكل لوحة الأدمن بس — مفيش حماية حقيقية على الصفحة دي هنا، وأي طلب بيظهر جوّاها اتبعت في نفس الجلسة دي بس (مش متخزن في قاعدة بيانات حقيقية).
      </div>
      <h2>طلبات التسجيل</h2>
      {students.length === 0 ? (
...
function LectureStage({ subject, lecture, idx, accessUnlocked, paced, watched, resultPercent, prevLecture, prevWatched, prevResult, isLast, onProgress, onOpenQuiz, onOpenPrevQuiz }) {
    ...
    <div className="lecture-stage">
      ...
      {fullyOpen && (
        <>
          ...
          <div className="lu-actions">
            <LecturePlayer url={lecture.videoUrl} durationMinutes={lecture.durationMinutes} initialWatched={watched} onProgress={onProgress} />
            <div className="lu-buttons">
              <span className="lu-chip"><Icon name="file" className="lu-mini" /> ملخص المحاضرة</span>
            </div>
          </div>
...
export default function App() {
  const [state, setState] = useState({ term: false, subjects: [], lectures: [], watched: {}, quizzesTaken: {} });
    ...
    (async () => {
      try {
        ...
        if (res && res.value) {
          ...
          setState({ term: !!parsed.term, subjects: parsed.subjects || [], lectures: parsed.lectures || [], watched: parsed.watched || {}, quizzesTaken: parsed.quizzesTaken || {} });
        }
  ...
  function redeem(rawCode) {
    ...
    if (!c) return;
    if (c === TERM_CODE) { persist({ ...state, term: true }); setMsg({ type: "ok", text: "تم فتح التيرم الأول كاملاً — كل المواد الأربع متاحة الآن." }); setCode(""); return; }
    ...
    if (subj) {
      ...
      else { persist({ ...state, subjects: [...state.subjects, subj.subjectId] }); const name = SUBJECTS.find((s) => s.id === subj.subjectId)?.name; setMsg({ type: "ok", text: `تم فتح مادة "${name}" بكل محاضراتها.` }); }
      setCode(""); return;
    }
    ...
    if (lec) {
      ...
      else { persist({ ...state, lectures: [...state.lectures, key] }); setMsg({ type: "ok", text: "تم فتح المحاضرة." }); setOpenSubject(lec.subjectId); }
      setCode(""); return;
    }
  ...
  async function resetAll() { await persist({ term: false, subjects: [], lectures: [], watched: {}, quizzesTaken: {} }); setMsg(null); setOpenSubject(null); }
...
.lu-chip { display: inline-flex; align-items: center; gap: 6px; font-size: 12.5px; background: var(--bg-panel-2); border: 1px solid var(--line); border-radius: 20px; padding: 7px 13px; color: var(--chalk-dim); }
.lu-chip-btn { border: none; color: var(--bg); background: var(--teal); font-weight: 700; }
.lu-chip-btn:hover { opacity: .9; }

.player-wrap { display: flex; flex-direction: column; gap: 6px; }
